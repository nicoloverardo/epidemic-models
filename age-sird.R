                                ...
                                R_0_end = 0.8, dataProvinces=dataProvinces)
{
  ...
  eventfun <- function(t, y, parms){
    with(as.list(y),
         {
           I1 <- I1 + real_cases[t]/3
           I2 <- I2 + real_cases[t]/3
           I3 <- I3 + real_cases[t]/3

           return(c(S1,S2,S3,I1,I2,I3,R1,R2,R3,D1,D2,D3))
         })
  }
  
  results <- as.data.frame(ode(y=inits, times=vt, func=calc_deriv, parms=parms, events = list(fun = eventfun, time=vt)))
